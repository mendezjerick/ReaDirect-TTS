...
Very short lines such as "Good job.", "Try again.", or "Correct." often sound flat because Kokoro receives too little context for natural rhythm. ReaDirect now expands safe agent narration into brief spoken lines that are still focused on the learning task.
...
2. Friendly Line Revamp / Text Humanization Layer
   - Implemented in `tts_humanizer.py`.
   - Expands known robotic lines using agent-specific templates.
   - Supports state, outcome, attempt count, protected terms, and request-level disable flags.
   - Uses deterministic variation so repeated cache keys stay stable.

3. Delivery Direction Layer
   - Implemented in `tts_humanizer.py`.
...
4. Audio Post-Processing / Humanizer Layer
...
```powershell
python scripts\compare_humanized_tts.py
```

The script posts the same lines twice: once with text/delivery/audio humanization disabled, and once with the default humanized pipeline. It saves both outputs under `generated_audio/humanization_comparison/`.
...
Pacing is mainly controlled by line rewriting, sentence punctuation, safe sentence chunking, and post-generation sentence pauses. Avoid adding ellipses everywhere; that usually sounds artificial.
